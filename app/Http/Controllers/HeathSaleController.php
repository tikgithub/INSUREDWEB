...
class HeathSaleController extends Controller
{
    ...
    public function showPaymentSubmitPage($id)
    {
        ...
        if (Session('health')) {
            ...
            inner join heath_cover_items hci on hci.id  = hpd.item_id
            where hpd.plan_id  = ?";
            $coverData = collect(DB::select($coverDataQuery, [Session('health')]));

            $plan = HeathPlan::find(Session('health'));
    ...
    public function submitHeathPayment(Request $req)
    {
        if (Session('health')) {
            ...
            $heathData->cus_pay_time = $req->input('transfer_time');

            $heathData->refer_no = $req->input('refer_no');

            $heathData->cus_amount = (float)str_replace(',', '', $req->input('transfer_amount'));
