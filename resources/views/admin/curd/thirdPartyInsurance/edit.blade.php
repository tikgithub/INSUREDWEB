...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb notosanLao">
        ...
        <li class="breadcrumb-item"><a href="{{ route('ThirdPartyInsuranceController.index') }}">ກຳນົດຮູບແບບປະກັນໄພບຸກຄົນທີ 3</a></li>
        <li class="breadcrumb-item active" aria-current="page"> ແກ້ໄຂຮູບແບບປະກັນໄພບຸກຄົນທີ 3</li>
    </ol>
</nav>
<hr>
<h3 class="notosanLao text-center">ແກ້ໄຂຮູບແບບປະກັນໄພບຸກຄົນທີ 3</h3>
<div class="row notosanLao">
    <div class="col-md-4 offset-md-4">
        ...
        <form action="{{route('ThirPartyInsuranceController.store')}}" method="post" class="fs-4">
            ...
            <div class="mb-3">
                ...
                <select name="level" id="level"
                    ...
                    @foreach ($levels as $item)
                        <option value="{{ $item->id }}" {{($package->level == $item->id)? 'selected':''}}>{{ $item->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="company" id="company"
                    ...
                    @foreach ($companies as $item)
                        <option value="{{ $item->id }}" {{($package->company_id == $item->id)? 'selected':''}}>{{ $item->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            {{-- php command --}}
            @php
                $vehicleTypeId = Vehicle_Detail::find($package->vehicle_detail);
            @endphp
            <div class="mb-3">
                ...
                <select name="vehicle_type" id="vehicle_type" class="form-select form-select-lg"
                    ...
                    @foreach ($vehicleTypes as $item)
                        <option value="{{ $item->id }}"  {{$vehicleTypeId->v_id == $item->id? 'selected':''}}> {{ $item->name }}</option>
                    @endforeach
                </select>
