...
@section('content')
    ...
    <nav aria-label="breadcrumb ">
        <ol class="breadcrumb notosanLao">
            ...
            <li class="breadcrumb-item active" aria-current="page">ລາຍທີ່ລູກຄ້າຈ່່າຍເງິນ</li>
        </ol>
    </nav>
    <hr>
    {{-- Body Data --}}
                    ...
                    <div class="row">
                        <div class="col-md-12 text-center notosanLao">
                            <form action="{{route('AdminController.approveInsurance',['id'=>$inputData->id])}}" method="post">
                                ...
                                <div class="mb-3 row">
                                    <label for="contract_no"
                                        class="col-sm-4 fs-4 text-center col-form-label">ໝົດສັນຍາວັນທີ</label>
                                    <div class="col-sm-8">
                                        <input type="text"
                                            class="form-control form-control-lg {{ $errors->has('start_date') ? 'border-danger' : '' }}"
                                            id="end_date" value="" readonly>
                                    </div>
                                </div>

                                <button type="submit" class="notosanLao btn-lg btn bg-blue text-white">
                                    <i class="bi bi-check-circle me-2"></i> ອານຸມັດລາຍການນີ້</button>
                            </form>
            ...
            <form autocomplete="off" method="POST" action="{{ route('AdminController.updateCustomerInsuranceInformation') }}"
                ...
                <fieldset class="border notosanLao">
                    ...
                    <div class="p-2">
                        ...
                        <div class="mb-3 row">
                            <div class="col-sm-6 text-center">
                                <img class="ms-2" src="{{ asset($inputData->rear_image) }}"
                                    style="width: auto; height: 100px;" alt="" srcset="">
                            </div>
                            <div class="col-sm-6 align-self-center">
                        ...
                        <div class="mb-3 row">
                            <div class="col-md-12">
                                <div class="form-check notosanLao">
                                    <input class="form-check-input ms-3" type="checkbox" value="" id="acceptCheckbox"
                                        onchange="onClickAccepted()">
                                    <label class="form-check-label ms-2 fw-bolder" for="acceptCheckbox">
                                        ຂ້ອຍຕ້ອງການແກ້ໄຂຂໍ້ມູນໃຫ້ລູກຄ້າ
                                    </label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3 row">
                            <div class="col-md-12 d-flex justify-content-center">
                                <button id="btnSubmit" type="submit" class=" btn-lg btn bg-blue text-white" disabled><i
                                        class="bi bi-pencil-square me-2"></i> ແກ້ໄຂ</button>
                            </div>
                        </div>
                    </div>
...
@section('scripting')
    <script>
        ...
        /*** Function to add 1 year to current date */
        function addMore1Year() {
            var startDate = document.getElementById('start_date');

            var endDate = document.getElementById('end_date');
            var signDate = new Date(startDate.value);

            endDate.value = (parseInt(signDate.getMonth())+1).length > 1  ? (parseInt(signDate.getMonth()) + 1) : '0' + (
                parseInt(signDate.getMonth()) + 1) + '/' + signDate.getDate() + '/' + (parseInt(signDate
            .getFullYear()) + 1);
        }
